# Run with sudo after manual cargo build - x86_64 only, no aarch64

PKG_ID := $(shell yq e ".id" manifest.yaml)
PKG_VERSION := $(shell yq e ".version" manifest.yaml)
TS_FILES := $(shell find ./ -name \*.ts)
EPIC_NODE_SRC := $(shell find ./epic-node)
...
docker-images/x86_64.tar: Dockerfile docker_entrypoint.sh epic-node/target/release/epic-node
ifeq ($(ARCH),aarch64)
else
